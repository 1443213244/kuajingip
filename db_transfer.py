...
class TransferBase(object):
	def __init__(self):
		...
		self.key_list = ['port', 'u', 'd', 'transfer_enable', 'passwd', 'enable', 'end_date', 'pid', 'main_ip', 'bandwidth', 'ceil', 'burst']
	...
	def del_server_out_of_bound_safe(self, last_rows, rows):
		...
		for row in rows:
			try:
				gateway = str(config['gateway']).split('.')
				main_ip = str(row['main_ip']).split('.')
				allow = switchrule.isTurnOn(row) and row['enable'] == 1 and datetime.datetime.now() <= row['end_date'] and gateway[0] == main_ip[0] and gateway[1] == main_ip[1] and gateway[2] == main_ip[2]
			except Exception as e:
				allow = False

			port = row['port']
			bandwidth = row['bandwidth']
			tpid = row['main_ip'].split('.')
			pid = tpid[3]
			ceil = row['ceil']
			burst = row['burst']
			...
			if port in mu_servers:
				if ServerPool.get_instance().server_is_run(port) > 0:
					...
					self.new_server(port, pid, device, bandwidth, ceil, burst, passwd, cfg)
	...
	def new_server(self, port,  pid, device, bandwidth, ceil, burst, passwd, cfg):
		...
		os.system('tc class add dev %s parent 1: classid 1:%s htb rate %s ceil %s burst %s prio 0' % (device, pid, str(bandwidth) + 'mbit', str(ceil) + 'mbit', str(burst) + 'k'))
		...
		ServerPool.get_instance().new_server(port, pid, bandwidth, cfg)
